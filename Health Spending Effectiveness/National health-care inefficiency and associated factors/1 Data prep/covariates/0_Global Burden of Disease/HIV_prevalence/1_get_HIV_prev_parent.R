...
### Purpose: Parent script for extracting HIV prevalence draws in parallel.
###          Calls 0_get_HIV_prev_child.R in a loop over all locations individually.
###########################################################################
...
for (loc in loc_ids) {
  ...
  sbatch_str <- paste("sbatch -J",job_name,"--mem=10G -c 2 -A proj_fgh -t 02:00:00 -p all.q",
