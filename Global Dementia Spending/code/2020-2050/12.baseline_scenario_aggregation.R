...
sc_path <- "ADDRESS"
 ...
 loc <- commandArgs()[8]
  ...
  gamma <- fread(paste0(bl_path, "FILEPATH", loc, "FILEPATH"))
  

  # Merging together
  data <- merge(data, gamma, by = c("location_id", 'location_name','year_id',"draw"), all.x = T)
  
  ##
  # Reading theta
  ##
  
  theta <- fread(paste0(bl_path, "FILEPATH", loc, "FILEPATH"))
  ...
  #########################################################################################
  ##
  # calculating people with dementia & diagnosis either living in the community 
  ##
  #########################################################################################
  ...
  ##
  # Calculating people with dementia who live in nursing homes
  ##
  
  data[, pre_dx_ins_counts := pre_dx_counts * gamma] # Institution with dx
  
  data[, pre_no_dx_ins_counts := pre_no_dx_counts * theta] # Institution with no dx
  ...
  f_costs <- merge(fac_l, inst_c, by = c('location_id', 'year_id', 'draw', 'dx_status'), 
                   allow.cartesian = T)
  ...
  ##
  # Finally, calculating dementia spending!! 
  ##
  ...
  # removing duplicates
  spend_s <- unique(spend[dx_status == 'dx', c('location_id', 'year_id', 'draw', 
                                               'cost_type', 'agg_final_dem_costs')])
