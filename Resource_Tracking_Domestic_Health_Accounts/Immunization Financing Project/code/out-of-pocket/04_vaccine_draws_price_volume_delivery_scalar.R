########################################################################################
## Create OOP estimates for specific vaccines
...
volume <- fread(paste0(out.dir, "volume/draws_135_total_volume_vaccines_20200719.csv"))
head(volume)
...
price <- fread(paste0(out.dir, "price/filled_price_9_vaccines_MRseparate_20200629.csv"))
price[, draw := draw - 1]
...
delivery <- fread(paste0(out.dir, "delivery/modeled_delivery_costs_draws_20200626.csv"))
head(delivery)
...
scalar <- fread(paste0(out.dir, "oop_scalar/oop_scalar_stgpr_draws_20200628.csv"))
...
## -------------------------------------------------------------------------------------
## 4. Calculate OOP Spending by vaccine
## -------------------------------------------------------------------------------------
...
price_volume_scalar2 <- price_volume_scalar[!is.na(oop_spending), 
                                            ...
                                            .(ihme_loc_id, year_id, vaccine, draw)]
head(price_volume_scalar2[ihme_loc_id == 'AFG' & draw == 1 & year_id == 2000], 10)
## Number of unique country-year-vaccine counts (since not all vaccines are all years)
stop(nrow(price_volume_scalar2) == 16335000)
...
price_volume_scalar3 <- get_location_id(price_volume_scalar2)
...
length(unique(price_volume_scalar3$vaccine))
length(unique(price_volume_scalar3$draw))
nrow(price_volume_scalar3) == 16335000

length(unique(price_volume_scalar4$ihme_loc_id))
length(unique(price_volume_scalar4$year_id))
length(unique(price_volume_scalar4$vaccine))
nrow(price_volume_scalar4) == 16335

fwrite(price_volume_scalar4[ihme_loc_id %in% locs_lm$ihme_loc_id,], 
       paste0("FILEPATH/data_vaccine_OOP_by_country_", 
              date1, "_", tag, ".csv"))

fwrite(price_volume_scalar3[ihme_loc_id %in% locs_lm$ihme_loc_id, .
                            (location_id, ihme_loc_id, year_id, draw, 
                              oop_spending, routine_spending = oop_spending, 
                              supplem_spending = 0, vaccine_spending = oop_vac_spending,
                              delivery_spending = oop_del_spending)], 
       paste0("FILEPATH/draws_vaccine_OOP_by_country_", 
              date1, "_", tag, ".csv"))
...
## Check results
length(unique(price_volume_scalar_gbl$vaccine))
length(unique(price_volume_scalar_gbl$year_id))
nrow(price_volume_scalar_gbl) == 121

## Output results
fwrite(price_volume_scalar_gbl, 
       paste0("FILEPATH/gbl_vaccine_OOP_", 
              date1, "_", tag, ".csv"))
...
price_volume_scalar_gavi <- price_volume_scalar3[, .(oop_spending = sum(oop_spending),
                                                    ...
                                                    oop_vac_spending = sum(oop_vac_spending),
                                                    cv_surviving_infant_pop = sum(cv_surviving_infant_pop)),
                                                .(vaccine, year_id, gavi_eligible, draw)]
...
price_volume_scalar_gavi <- price_volume_scalar_gavi[, .(oop = mean(oop_spending),
                                                       ...
                                                       oop_vac_spi_lower = quantile(oop_vac_spending_per_infant, 0.025),
                                                       oop_vac_spi_upper = quantile(oop_vac_spending_per_infant, 0.975)),
                                                   .(vaccine, gavi_eligible, year_id)]

price_volume_scalar_gbl[year_id == 2017, .(vaccine,
                                           round(oop_spi, 2),
                                           round(oop_del_spi, 2),
                                           round(oop_vac_spi, 2))]
price_volume_scalar_gavi[year_id == 2017, .(vaccine, 
                                            gavi_eligible,
                                            round(oop_spi, 2),
                                            round(oop_del_spi, 2),
                                            round(oop_vac_spi, 2))]
...
nrow(price_volume_scalar_gavi) ==  2 * 121

## Output results
fwrite(price_volume_scalar_gavi, 
       paste0("FILEPATH/gavi_non-gavi_vaccine_OOP_", 
              date1, "_", tag, ".csv"))
