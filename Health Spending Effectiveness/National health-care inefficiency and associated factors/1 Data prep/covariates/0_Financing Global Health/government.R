##########################################################################
### Author: USERNAME
### Date: 04/08/2024
### Project: Health Spending Effectiveness 
### Purpose: Extracting government health expenditure data from FGH
###########################################################################

# clear environment
rm(list=ls())


# load libraries
library(data.table)
library(dplyr)
library(ggplot2)
library(feather)
# source functions 
source("FILEPATH/helper_functions.R")
source("FILEPATH/currency_conversion.R")
source(file.path("FILEPATH", "get_location_metadata.R"))
source(paste0("FILEPATH","get_population.R"))

################################### FGH 2024 ###################################
# initialize FGH2024 directory
fgh_dir = "FILEPATH"

# get government health expenditure data from feather file
ghe_totes = read_feather(file.path(fgh_dir, "ghe_totes_feather.feather"))

# convert to data.table
setDT(ghe_totes)

# filter to years of interest
ghe_totes = ghe_totes[year %in% c(1995:2022)]

# change names of FGH data to match GBD
setnames(ghe_totes, old = c("year", "iso3"), new = c("year_id", "ihme_loc_id"))

# pivot draws to long format
ghe_totes = melt(ghe_totes, 
                 id.vars = c("ihme_loc_id", "year_id"), 
                 variable.name = "draw", 
                 value.name = "ghe")

# get GBD location metadata
locs <- get_location_metadata(location_set_id=35, release_id=16) # GBD 2023

# filter to just national level
locs = locs[level == 3] # level 3 -> country

# merge with location metadata
ghe_totes = merge(ghe_totes, locs[, .(location_id, ihme_loc_id, 
                                      location_name, super_region_name)], 
                  by = "ihme_loc_id")

# convert currency from 2023 USD to 2023 PPP
ghe_ppp = currency_conversion(ghe_totes, 
                              col.loc = "ihme_loc_id", # ISO3 codes
                              col.value = "ghe", # name of column with values to convert
                              currency = "usd", # current currency unit
                              currency.year = 2023, # current currency year
                              base.year = 2023, # year of currency to convert to
                              base.unit = "ppp") # unit of currency to convert to

# convert to per capita with population estimates: 
pop <- get_population(release_id = 16, # GBD 2023
                      location_id = unique(ghe_ppp$location_id),
                      year_id = unique(ghe_ppp$year_id), 
                      sex_id = 3, # 3 -> all sexes
                      age_group_id = 22) # 22 -> all ages

# remove unnecessary column
pop[, run_id := NULL]

# merge population with government health expenditure data
ghe_ppp = merge(ghe_ppp, pop, 
                by = c("year_id", "location_id"))

# calculate ghe per capita: 
ghe_ppp[, ghe_pc := ghe / population]

# create per capita values in USD, not PPP, and save out
ghe_usd = merge(ghe_totes, pop, 
                by = c("year_id", "location_id"))
ghe_usd[, ghe_pc := ghe / population]

# save out ghe in USD data:
output_dir = "FILEPATH"
fwrite(ghe_usd, paste0(output_dir, "ghe_usd23_1995_2022.csv"))

# save out ghe in PPP data: 
fwrite(ghe_ppp, paste0(output_dir, "ghe_ppp23_1995_2022.csv"))


################################### FGH 2023 ###################################
# get government health expenditure data
ghes_totes = get_he_data("ghes_totes", full = TRUE, fgh_round_id = 16) # FGH 2023
# explore what data this pulls: 
length(unique(ghes_totes$location_id)) # 204
range(ghes_totes$year_id) # 1995 - 2050
range(ghes_totes$draw) # 1 - 500

# filter to 1995-2023
ghes_totes = ghes_totes[year_id %in% c(1995:2021)]

# convert currency from 2022 USD to 2021 PPP
ghes_ppp = currency_conversion(ghes_totes, 
                               col.loc = "ihme_loc_id", # ISO3 codes
                               col.value = "data_var", # name of column with values to convert
                               currency = "usd", # current currency unit
                               currency.year = 2022, # current currency year
                               base.year = 2021, # year of currency to convert to
                               base.unit = "ppp") # unit of currency to convert to

# convert to per capita: 
# source shared functions
source(paste0(functions_dir,"get_population.R"))
source(paste0(functions_dir,"get_location_metadata.R"))

# get location IDs from metadata
locs <- get_location_metadata(location_set_id=35, release_id = 16) 
# location_set_id 35 -> Model Results 
# release_id 16 -> GBD 2023
# get country IDs 
country_ids <- locs[level == 3]$location_id # level 3 -> country

# get population for all ages and sexes for all countries
pop <- get_population(release_id = 16, # GBD 2023
                      location_id = country_ids,
                      year_id = c(1995:2021), 
                      sex_id = 3, # 3 -> all sexes
                      age_group_id = 22) # 22 -> all ages

# remove unnecessary column
pop[, run_id := NULL]

# merge population with government health expenditure data
ghes_ppp = merge(ghes_ppp, pop, 
                 by = c("year_id", "location_id"))

# calculate GHE per capita: 
ghes_ppp[, ghes_pc := data_var / population]

# change data_var name
setnames(ghes_ppp, old = "data_var", new = "ghes_totes")

# save out GHE PPP data: 
output_dir = "FILEPATH"
fwrite(ghes_ppp, paste0(output_dir, "GHES_pc_ppp_1995_2021_fgh2023.csv"))

