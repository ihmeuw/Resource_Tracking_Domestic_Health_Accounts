# Plot community results. Script adapted from 
...
# read in data
raw = fread("FILEPATH/11_09_23_community_data.csv")
stage1 = fread("FILEPATH/11_09_23_community_pred.csv")
...
output_dir = "FILEPATH"
pdf(paste0(output_dir,"global_spending_community_plots_run_with_raw",model_num,".pdf"))
for(country in country_ids[location_id %in% countries_with_raw,]$location_id){
  ...
  p = ggplot(country_df, aes(x = year_id, y = spending, 
    ...
    labs(x = "Year", y = "Spending (community)",
...
output_dir = "FILEPATH"
pdf(paste0(output_dir,"global_spending_community_plots_run_without_raw",model_num,".pdf")) 
