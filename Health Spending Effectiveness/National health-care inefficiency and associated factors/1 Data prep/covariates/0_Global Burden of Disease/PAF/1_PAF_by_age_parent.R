...
### Purpose: Parent script for extracting population attributable fraction draws by age group in parallel.
###          Calls 0_get_PAF_by_age_child.R in a loop over all locations individually.
###########################################################################
...
child_script = "FILEPATH/0_get_PAF_by_age_child.R"
...
for (loc in loc_ids) {
  ...
  sbatch_str <- paste("sbatch -J",job_name,"--mem=10G -c 2 -A proj_fgh -t 02:00:00 -p long.q",
                      "-o ", output_log," -e", error_log, 
                      "FILEPATH/execR.sh -i FILEPATH/ihme_rstudio_4401.img",
