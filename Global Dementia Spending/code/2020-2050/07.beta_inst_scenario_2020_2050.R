...
for (loc in locs){
  ...
  # loading beta to make sure beta and beta inst are aligned
  beta <- fread(paste0(out_root, 'FILEPATH',loc, 'FILEPATH'))
  
  x_1 <- x_sce[location_id == loc, c('location_id', 'location_name', 'draw', 
  ...
  x_1l[, year := tstrsplit(variable, "_", keep = 3)]
  x_1l[, year_id := as.numeric(paste0(20, year))]
  x_1l[, year := NULL]  
  x_1l[, variable := NULL]
  
  data <- merge(beta, x_1l, by = c('location_id', 'draw', 'location_name', 'year_id'))
  ...
  fwrite(data_f,
         paste0(out_root, 'FILEPATH', loc, 'FILEPATH'),
         row.names = F)
